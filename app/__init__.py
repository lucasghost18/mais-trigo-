...
def create_app(test_config=None):
    ...
    with app.app_context():
        ...
        try:
            ...
            admin = User.query.filter_by(username='mais vendas').first()
            if not admin:
                admin = User(username='mais vendas', password='3341', role='admin', vendor_id=None)
                db.session.add(admin)
                db.session.commit()
                app.logger.info('Admin user seeded: mais vendas')
        except Exception as e:
            app.logger.info(f'Admin seed skipped or failed: {e}')
